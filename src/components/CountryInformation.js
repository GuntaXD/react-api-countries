  ...
  if (
    ...
    if (country.hasOwnProperty("borders")) {
      borders = country.borders.map((border) => {
        return props.countries.find((element) => element.cca3 === border).name
          .common;
      });
    }
    ...
    return (
      <>
        <Link to="/" className="backLink" onClick={props.backOnClick}>
          Back
        </Link>
        <div className="countryInformation">
          <img className="countryImg" src={country.flags.svg} alt=""></img>
          <div className="countryInfo">
            <h1 className="countryInfo__countryName">{country.name.common}</h1>
